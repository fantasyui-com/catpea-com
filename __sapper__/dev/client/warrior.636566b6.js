...
function create_each_block$1(ctx) {
	...
	let t;
	let current;
	const standard_spread_levels = [/*item*/ ctx[6]];
	let standard_props = {};

	for (let i = 0; i < standard_spread_levels.length; i += 1) {
		standard_props = assign(standard_props, standard_spread_levels[i]);
	}

	const standard = new Standard({ props: standard_props, $$inline: true });

	const block = {
		c: function create() {
			div = element("div");
			create_component(standard.$$.fragment);
			t = space();
			this.h();
		},
		l: function claim(nodes) {
			...
			var div_nodes = children(div);
			claim_component(standard.$$.fragment, div_nodes);
			t = claim_space(div_nodes);
			div_nodes.forEach(detach_dev);
			this.h();
		},
		...
		m: function mount(target, anchor) {
			insert_dev(target, div, anchor);
			mount_component(standard, div, null);
			append_dev(div, t);
			current = true;
		},
		p: function update(ctx, dirty) {
			const standard_changes = (dirty & /*content*/ 4)
			? get_spread_update(standard_spread_levels, [get_spread_object(/*item*/ ctx[6])])
			: {};

			standard.$set(standard_changes);
		},
		i: function intro(local) {
			if (current) return;
			transition_in(standard.$$.fragment, local);
			current = true;
		},
		o: function outro(local) {
			transition_out(standard.$$.fragment, local);
			current = false;
		},
		d: function destroy(detaching) {
			if (detaching) detach_dev(div);
			destroy_component(standard);
		}
	};
