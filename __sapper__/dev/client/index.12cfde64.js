...
var data$3 = getCjsExportFromNamespace(data$2);

for (const key of Object.keys(data$3)) {
  // Returns a string representation of html attributes
  const htmlAttributes = (icon, options) => {
    const attributes = [];
    const attrObj = objectAssign({}, data$3[key].options, options);
    ...
    if (options) {
      ...
      if (options['width'] || options['height']) {
        attrObj['width'] = options['width']
          ? options['width']
          : (parseInt(options['height']) * data$3[key].options['width']) / data$3[key].options['height'];
        attrObj['height'] = options['height']
          ? options['height']
          : (parseInt(options['width']) * data$3[key].options['height']) / data$3[key].options['width'];
      }
...
function create_if_block_1(ctx) {
	...
	const block = {
		...
		l: function claim(nodes) {
			...
			t0 = claim_text(div0_nodes, /*title*/ ctx[3]);
			...
			t3 = claim_text(p_nodes, /*advice*/ ctx[4]);
		...
		m: function mount(target, anchor) {
			...
			dispose = [
				...
				listen_dev(p, "click", boom, false, false, false)
			];
		},
...
function create_if_block(ctx) {
	...
	const block = {
		...
		h: function hydrate() {
			attr_dev(button, "class", "btn btn-text btn-sm border border-secondary float-right");
			add_location(button, file$1, 58, 6, 998);
			attr_dev(div0, "class", "card-header lead");
			add_location(div0, file$1, 56, 4, 916);
			attr_dev(label, "class", "small");
			attr_dev(label, "for", "originalBeat");
			add_location(label, file$1, 63, 6, 1220);
...
*/
	...
	$$self.$inject_state = $$props => {
		...
		if ("advice" in $$props) $$invalidate(4, advice = $$props.advice);
	};
